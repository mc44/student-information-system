...
my_tree['columns'] = ("ID", "Name", "Course", "Year", "Gender")
...
def editOrDeleteEntry(delete=FALSE):
    ...
    for line in data_screenshot:
        if str(line[0]) == sid and str(line[1]) == name and str(line[2]) == course and str(line[3]) == year and str(line[4]) == gender:
            break
        counter += 1
    c = data_screenshot[counter]
    if str(s_tb) == str(c[0]) and str(n_tb) == str(c[1]) and str(c_tb) == str(c[2]) and str(y_tb) == str(c[3]) and str(g_tb) == str(c[4]) and not delete:
        return
    ...
    if delete:
        ...
        data_screenshot[counter] = [tb_id.get(), tb_name.get(), tb_course.get(), tb_year.get(), tb_gender.get()]
...
def addEntry(button):
    ...
    def validate():
        ...
        if checkdb(t1):
            ...
            return
        if not re.fullmatch(r"\d\d\d\d-\d\d\d\d", t1):
            messagebox.showinfo(parent=addscreen, title="ID Exists", message="Inputted ID is not in format ####-####")
            return
        my_tree.insert(parent='', index='end', iid=len(my_tree.get_children())+1, text="", values=(t1, t2, t3, t4, t5))
    ...
    label5 = ttk.Label(addscreen, text="Gender")
    ...
    tbox5 = ttk.Entry(addscreen, width=15)
    ...
    tbox5.place(relx=0.25, rely=0.645, relheight=0.11, relwidth=0.72)
...
lab_gender.place(relx=0.1, rely=0.505+pdown, relheight=0.06, relwidth=0.1)
...
tb_gender.place(relx=0.25, rely=0.505+pdown, relheight=0.08, relwidth=0.72)
